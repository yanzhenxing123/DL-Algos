...
class ModelConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            n_kv_heads: int = 8, # 键值头的数量，用于分组查询注意力 GQA：多个 Q 头共享同一组 K/V 头。
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    # GQA: 将每个 KV 头重复 n_rep 次，以匹配 Q 头的数量
    # 输入: [batch_size, seq_len, n_kv_heads, head_dim]
    # 输出: [batch_size, seq_len, n_kv_heads * n_rep, head_dim]
    
    # 步骤1: 在 KV 头维度后添加一个新维度，用于重复
    # [batch_size, seq_len, n_kv_heads, 1, head_dim]
    x_expanded = x[:, :, :, None, :] # torch.Size([1, 50, 8, 1, 48])
    
    # 步骤2: 将新维度扩展到 n_rep，实现重复
    # [batch_size, seq_len, n_kv_heads, n_rep, head_dim]
    x_repeated = x_expanded.expand(bs, slen, n_kv_heads, n_rep, head_dim)
    
    # 步骤3: 重塑，将重复的 KV 头合并到同一个维度
    # [batch_size, seq_len, n_kv_heads * n_rep, head_dim]
    x_reshaped = x_repeated.reshape(bs, slen, n_kv_heads * n_rep, head_dim)
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    """
    这是一个实现旋转位置编码的函数，通常用于像LLaMA、GPT-NeoX等现代Transformer模型中。让我详细解释它的作用和原理。
    这个函数预先计算旋转位置编码，用于为Transformer模型提供位置信息，让模型能够理解单词在序列中的顺序关系。
    """
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor):
        ...
        """
        xq.shape = torch.Size([1, 50, 768])
        xk.shape = xv.shape = torch.Size([1, 50, 384])
        
        """
        # 调整形状以适应头的维度。
        xq = xq.view(bsz, seqlen, self.n_local_heads, self.head_dim)  # torch.Size([1, 50, 16, 48])
        xk = xk.view(bsz, seqlen, self.n_local_kv_heads, self.head_dim) # torch.Size([1, 50, 8, 48])
        xv = xv.view(bsz, seqlen, self.n_local_kv_heads, self.head_dim)

        # 应用旋转位置嵌入（RoPE）。
        xq, xk = apply_rotary_emb(xq, xk, freqs_cos, freqs_sin) # 维度保持不变

        # 对键和值进行扩展以适应重复次数。
        xk = repeat_kv(xk, self.n_rep) # torch.Size([1, 50, 16, 48])
        xv = repeat_kv(xv, self.n_rep) # torch.Size([1, 50, 16, 48])
...
x = torch.randint(0, 6144, (2, 50)) # [bs, seq_len] 1, 50
# 实例化LLaMA2Model
