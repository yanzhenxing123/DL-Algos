"""
Wide 部分：负责简单、可解释的规则（需要保留原始特征）。Wide部分主要用于捕捉特征的交互作用（即高阶特征交互），
通常适用于具有高效数值映射的离散特征。Wide部分能够通过学习特征之间的线性关系来提升模型的效果。适合放入Wide部分的特征通常包括：

类别特征（Categorical features）：这些特征通常是离散的，比如用户ID、商品ID、地区、性别等。在Wide部分，这些特征会通过One-hot编码或Embedding表示，以捕捉特征之间的交互作用。
稀疏特征：例如，广告点击、用户行为等，这些特征在数据中可能呈现稀疏分布，但是通过线性模型能有效地进行预测。
稀疏的高维离散特征：如用户的历史购买记录、标签等，这些特征在宽度方向上使用One-hot编码可以获得良好的效果。
具有较强先验知识的特征：如果对某些特征有明确的线性关系预期，可以放入Wide部分来增强模型对这些关系的学习。
